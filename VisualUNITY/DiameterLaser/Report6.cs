...
namespace VisualUNITY
{
    public partial class Report6 : Form
    {
        ...
        public Report6(string idReceived, List<int> columnIdList)
        {
            InitializeComponent();
        ...
        private void Report1_6_Load(object sender, EventArgs e)
        {
            setParameters();
            ...
            SqlCommand command = new SqlCommand(string.Format("INSERT INTO dbo.Report6 (Param1, Param2, Param3, Param4, Param5, Param6) " +
                "SELECT {1}, {2}, {3}, {4}, {5}, {6} FROM {0}",
                dataTable.Rows[0][7].ToString(), columnNames[0], columnNames[1], columnNames[2], columnNames[3], columnNames[4],
                columnNames[5]), localDbConnection);
            command.ExecuteNonQuery();

            SqlDataAdapter dA = new SqlDataAdapter("SELECT * FROM dbo.Report6", localDbConnection);
        ...
        private void ThreadTask()
        {
            if (localDbConnection.State == ConnectionState.Closed)
                localDbConnection.Open();
            SqlCommand command = new SqlCommand("DELETE FROM dbo.Report6", localDbConnection);
            command.ExecuteNonQuery();
            localDbConnection.Close();
        }
