...
namespace VisualUNITY
{
    public partial class Report7 : Form
    {
        ...
        public Report7(string idReceived, List<int> columnIdList)
        {
            InitializeComponent();
        ...
        private void Report1_7_Load(object sender, EventArgs e)
        {
            setParameters();
            ...
            SqlCommand command = new SqlCommand(string.Format("INSERT INTO dbo.Report7 (Param1, Param2, Param3, Param4, Param5, Param6, Param7) " +
                "SELECT {1}, {2}, {3}, {4}, {5}, {6}, {7} FROM {0}",
                dataTable.Rows[0][7].ToString(), columnNames[0], columnNames[1], columnNames[2], columnNames[3], columnNames[4],
                columnNames[5], columnNames[6]), localDbConnection);
            command.ExecuteNonQuery();

            SqlDataAdapter dA = new SqlDataAdapter("SELECT * FROM dbo.Report7", localDbConnection);
        ...
        private void ThreadTask()
        {
            if (localDbConnection.State == ConnectionState.Closed)
                localDbConnection.Open();
            SqlCommand command = new SqlCommand("DELETE FROM dbo.Report7", localDbConnection);
            command.ExecuteNonQuery();
            localDbConnection.Close();
        }
