...
namespace VisualUNITY
{
    public partial class Report1 : Form
    {
        ...
        public Report1(string idReceived, List<int> columnIdList)
        {
            InitializeComponent();
        ...
        private void Report1_1_Load(object sender, EventArgs e)
        {
            setParameters();
            ...
            SqlCommand command = new SqlCommand(string.Format("INSERT INTO dbo.Report1 (Param1) SELECT {1} FROM {0}",
                dataTable.Rows[0][7].ToString(), columnNames[0]), localDbConnection);
            command.ExecuteNonQuery();

            SqlDataAdapter dA = new SqlDataAdapter("SELECT * FROM dbo.Report1", localDbConnection);
        ...
        private void ThreadTask()
        {
            if (localDbConnection.State == ConnectionState.Closed)
                localDbConnection.Open();
            SqlCommand command = new SqlCommand("DELETE FROM dbo.Report1", localDbConnection);
            command.ExecuteNonQuery();
            localDbConnection.Close();
        }
        ...
        private void init()
        {
            ...
            collection.Add(new ReportParameter("header", columnNames[0]));
