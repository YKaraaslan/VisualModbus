...
namespace VisualUNITY
{
    public partial class Report3 : Form
    {
        ...
        public Report3(string idReceived, List<int> columnIdList)
        {
            InitializeComponent();
        ...
        private void Report1_3_Load(object sender, EventArgs e)
        {
            setParameters();
            ...
            SqlCommand command = new SqlCommand(string.Format("INSERT INTO dbo.Report3 (Param1, Param2, Param3) SELECT {1}, {2}, {3} FROM {0}",
                dataTable.Rows[0][7].ToString(), columnNames[0], columnNames[1], columnNames[2]), localDbConnection);
            command.ExecuteNonQuery();

            SqlDataAdapter dA = new SqlDataAdapter("SELECT * FROM dbo.Report3", localDbConnection);
        ...
        private void ThreadTask()
        {
            if (localDbConnection.State == ConnectionState.Closed)
                localDbConnection.Open();
            SqlCommand command = new SqlCommand("DELETE FROM dbo.Report3", localDbConnection);
            command.ExecuteNonQuery();
            localDbConnection.Close();
        }
