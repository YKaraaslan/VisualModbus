...
namespace VisualUNITY
{
    public partial class Report8 : Form
    {
        ...
        public Report8(string idReceived, List<int> columnIdList)
        {
            InitializeComponent();
        ...
        private void Report1_8_Load(object sender, EventArgs e)
        {
            setParameters();
            ...
            SqlCommand command = new SqlCommand(string.Format("INSERT INTO dbo.Report8 (Param1, Param2, Param3, Param4, Param5, Param6, Param7, Param8) " +
                "SELECT {1}, {2}, {3}, {4}, {5}, {6}, {7}, {8} FROM {0}",
                dataTable.Rows[0][7].ToString(), columnNames[0], columnNames[1], columnNames[2], columnNames[3], columnNames[4],
                columnNames[5], columnNames[6], columnNames[7]), localDbConnection);
            command.ExecuteNonQuery();

            SqlDataAdapter dA = new SqlDataAdapter("SELECT * FROM dbo.Report8", localDbConnection);
        ...
        private void ThreadTask()
        {
            if (localDbConnection.State == ConnectionState.Closed)
                localDbConnection.Open();
            SqlCommand command = new SqlCommand("DELETE FROM dbo.Report8", localDbConnection);
            command.ExecuteNonQuery();
            localDbConnection.Close();
        }
