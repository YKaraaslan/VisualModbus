...
namespace VisualUNITY
{
    public partial class Report2 : Form
    {
        ...
        public Report2(string idReceived, List<int> columnIdList)
        {
            InitializeComponent();
        ...
        private void Report1_2_Load(object sender, EventArgs e)
        {
            setParameters();
            ...
            SqlCommand command = new SqlCommand(string.Format("INSERT INTO dbo.Report2 (Param1, Param2) SELECT {1}, {2} FROM {0}", 
                dataTable.Rows[0][7].ToString(), columnNames[0], columnNames[1]), localDbConnection);
            command.ExecuteNonQuery();

            SqlDataAdapter dA = new SqlDataAdapter("SELECT * FROM dbo.Report2", localDbConnection);
        ...
        private void ThreadTask()
        {
            if (localDbConnection.State == ConnectionState.Closed)
                localDbConnection.Open();
            SqlCommand command = new SqlCommand("DELETE FROM dbo.Report2", localDbConnection);
            command.ExecuteNonQuery();
            localDbConnection.Close();
        }
